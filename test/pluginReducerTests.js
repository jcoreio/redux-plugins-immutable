...
import pluginReducer from '../src/pluginReducer'

import {NOT_LOADED, LOADED} from '../src/pluginTypes'
...
describe('pluginReducer', () => {
  ...
  describe('replacePlugin handler', () => {
    it("adds plugin iff present", () => {
      ...
      state = pluginReducer(state, replacePlugin(plugin))
      expect(state.toJS().plugins.p1).toEqual(jasmine.objectContaining(plugin.toJS()))

      let plugin2 = Immutable.fromJS({
        key: 'p2',
        name: 'Plugin 2'
      })

      state = pluginReducer(state, replacePlugin(plugin2))
      expect(state.toJS().plugins.p2).toBe(undefined)
    })
    it("sets loadStatus to NOT_LOADED if missing", () => {
      let state = Immutable.fromJS({
        plugins: {
